function Plot_Vel(Table, plotName,Axis, Lim)
...
if contains(Axis,'x')
    plot(time_vec(window)', Table.LINEAR_ACC_VEL_X(window), 'LineWidth', lineW)
    legtxt{count} = 'X';
    count = count +1;
end
hold on
if contains(Axis,'y')
    plot(time_vec(window)', Table.LINEAR_ACC_VEL_Y(window), 'LineWidth', lineW)
    legtxt{count} = 'Y';
    count = count +1;
end
if contains(Axis,'z')
    plot(time_vec(window)', Table.LINEAR_ACC_VEL_Z(window), 'LineWidth', lineW)
    legtxt{count} = 'X';
    count = count +1;
end
hold off
title(getfield(names,plotName), 'fontsize',labelFont*1.5);
ylabel('Velocity [$\frac{m}{sec}$]', 'Interpreter','latex',... 
    'fontsize',labelFont);
...
figName = ['/figures/',plotName,'_velocity.fig'];
